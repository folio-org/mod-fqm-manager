...
if (process.argv.length < 3) {
  console.error('Usage: bun scripts/dump-entity-type-information.ts <label>');
  process.exit(1);
}
...
for (const { id, label } of entityTypes) {
  console.log('Dumping information for entity type', label);
  const filename = `${dir}/${label.replace(/[^a-z0-9]+/gi, '-').toLowerCase()}.csv`;
  ...
  await Bun.write(
    ...
    unparse(
      data.map((r) => ({
        ...
        'Showable in results': r.inResults,
        'API Only': r.apiOnly,
