...
public class EntityTypeRepository {
  ...
  public static final String customFieldTypeGetter = "jsonb ->> 'type'";
  ...
  public List<EntityTypeColumn> fetchNamesForSingleCheckbox(UUID entityTypeId) {
    ...
    return jooqContext
      .select(field(requiredFieldName), field(refId), field(customFieldTypeGetter))
      .from(sourceViewName)
      .where()
      .fetch()
      .stream()
      .map(record -> {
        ...
        Object customFieldType = record.get(2);
        if (SINGLE_CHECKBOX.equals(customFieldType)) {
          assert value != null;
          return handleSingleCheckboxCustomField(value.toString(), refId1.toString());
        } else {
          return null;
        }
      })
...
//    String customFieldSourceView = getEntityTypeDefinition(UUID.fromString(customFieldsEntityTypeId));
  ...
  private EntityTypeColumn handleSingleCheckboxCustomField(String value, String refId) {
