    ...
    return List.of(
      Arguments.of(
        ...
        MigratableQueryInformation
          .builder()
          .entityTypeId(TEST_ENTITY_TYPE_ID)
          .fqlQuery("{\"$and\": [{\"field1\": {\"$eq\": \"value1\"}}, {\"field2\": {\"$nin\": [\"value2\", \"value3\"]}}]}")
          .fields(List.of())
          .build()
      ),
      Arguments.of(
      ...
      Arguments.of(
        ...
        MigratableQueryInformation
          ...
          .entityTypeId(TEST_ENTITY_TYPE_ID)
          .fqlQuery(
            "{ \"$and\": [ { \"arrayField\": { \"$not_contains_any\": [\"value1\", \"value2\"] } }, { \"arrayField\": { \"$ne\": \"exclude\" } } ] }"
          )
          .fields(List.of())
          .build(),
        MigratableQueryInformation
          ...
          .entityTypeId(TEST_ENTITY_TYPE_ID)
          .fqlQuery(
            "{ \"$and\": [ { \"arrayField\": { \"$nin\": [\"value1\", \"value2\"] } }, { \"arrayField\": { \"$ne\": \"exclude\" } } ] }"
          )
          .fields(List.of())
          .build()
      ),
      ...
      Arguments.of(
        "Empty query",
        MigratableQueryInformation.builder().entityTypeId(TEST_ENTITY_TYPE_ID).fqlQuery("{}").fields(List.of()).build(),
        MigratableQueryInformation.builder().entityTypeId(TEST_ENTITY_TYPE_ID).fqlQuery("{}").fields(List.of()).build()
      )
    );
  }
