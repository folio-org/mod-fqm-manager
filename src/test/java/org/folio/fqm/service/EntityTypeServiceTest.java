...
class EntityTypeServiceTest {
  ...
  @Test
  void shouldFallBackToJsonLanguageLabelsBeforeJavaLocaleWhenTranslationsAreMissing() {
    UUID entityTypeId = UUID.randomUUID();
    List<String> tenantList = List.of(TENANT_ID);
    String valueColumnName = "languages";
    EntityType entityType = new EntityType()
  ...
  void updateCustomEntityType_shouldValidateCustomEntityType() {
    ...
    doThrow(new InvalidEntityTypeDefinitionException("invalid", entityTypeId))
      .when(entityTypeValidationService).validateCustomEntityType(eq(entityTypeId), any(CustomEntityType.class));
