...
class FqlToSqlConverterServiceTest {
  ...
  void setup() {
    ...
    entityType = MarcFieldFactory.addSyntheticColumns(
      ...
      List.of("marc_245", "marc_245_a", "marc_245_ind1", "marc_245_ind1_7_a", "marc_245_ind1_blank_a",
        "marc_245_ind1_1_ind2", "marc_245_ind1_1_ind2_0_a"),
      "diku"
    );
  }
  ...
  static List<Arguments> jooqConditionsSource() {
    ...
    return Arrays.asList(
      ...
      Arguments.of(
        "contains MARC constrained subfield",
        """
      ...
      Arguments.of(
        "equals MARC dual indicator (constrain one, query the other)",
        """
          {"marc_245_ind1_1_ind2": {"$eq": "0"}}""",
        condition(
          "exists (select 1 from diku_mod_source_record_storage.marc_indexers marc where marc.marc_id = \"record_lb\".matched_id and marc.field_no = '245' and lower(marc.ind1) = '1' and lower(marc.ind2) = {0})",
          field("lower(:value)", String.class, param("value", "0"))
        )
      ),
      Arguments.of(
        "equals MARC dual indicator blank target token",
        """
          {"marc_245_ind1_1_ind2": {"$eq": "blank"}}""",
        condition(
          "exists (select 1 from diku_mod_source_record_storage.marc_indexers marc where marc.marc_id = \"record_lb\".matched_id and marc.field_no = '245' and lower(marc.ind1) = '1' and lower(marc.ind2) = {0})",
          field("lower(:value)", String.class, param("value", "#"))
        )
      ...
      Arguments.of(
        "contains MARC dual constrained subfield",
        """
          {"marc_245_ind1_1_ind2_0_a": {"$contains": "Shakespeare"}}""",
        condition(
          "exists (select 1 from diku_mod_source_record_storage.marc_indexers marc where marc.marc_id = \"record_lb\".matched_id and marc.field_no = '245' and lower(marc.ind1) = '1' and lower(marc.ind2) = '0' and marc.subfield_no = 'a' and lower(marc.value) like {0})",
          DSL.concat(inline("%"), field("lower(:value)", String.class, param("value", "Shakespeare")), inline("%"))
        )
      ),
      Arguments.of(
